...
static int CalcBrowsePageDirAndFilesLayout(void)
{
    ...
    int iNumPerCol;//一列能放多少个目录
	int iNumPerRow;//一行能放多少个目录
    int iDeltaX;//一行里目录之间的间隔之和
	int iDeltaY;//一列里目录之间的间隔之和
    int i, j, k = 0;
                ...
                else /* "目录和文件图标" */
                {
                    ...
                    else  /* "选择"按钮不被按下时, 单击目录则进入, bUsedToSelectDir为0时单击文件则显示它 */
                    {
                        ...
                        else if (bUsedToSelectDir == 0) /* bUsedToSelectDir为0时单击文件时显示它 */
                        {
							...
							{
								OpenTextFile(tPageParams.strCurPictureFile);
