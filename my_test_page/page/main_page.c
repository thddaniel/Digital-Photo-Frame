...
static void  CalcMainPageLayout(PT_PageLayout ptPageLayout)
{
	...
	int iTmpTotalBytes; 	/*一个图标所占的大小单位字节*/
	PT_Layout atLayout;
	...
	atLayout[0].iTopLeftY  = iStartY;	//iYres * (1 / 10)
	atLayout[0].iBotRightX = atLayout[0].iTopLeftX + iWidth * 2 - 1; //iWidth * 2 宽是长的两倍
