...
int AllocVideoMem(int iNum)
{
	...
	/* 确定VideoMem的大小
	 */
	GetDispResolution(&iXres, &iYres, &iBpp);/*获得所使用的显示设备的分辨率和BPP*/
	iVMSize = iXres * iYres * iBpp / 8;      /*总共占据多少字节*/
	iLineBytes = iXres * iBpp / 8;           /* 一行数据占据多少字节 */
	...
	/* 指向framebuffer */
	ptNew->tPixelDatas.aucPixelDatas = g_ptDefaultDispOpr->pucDispMem;/* 象素数据存储的地方 */
	
	ptNew->iID = 0;
	ptNew->bDevFrameBuffer = 1;        /* 表示这个VideoMem是设备本身的framebuffer, 而不是用作缓存作用的VideoMem */
	ptNew->eVideoMemState  = VMS_FREE; /*VideoMem即framebuffer                                空闲的*/
	ptNew->ePicState	   = PS_BLANK; /* VideoMem中内存里图片的状态    空白的*/
	...
	for (i = 0; i < iNum; i++)
	{
		...
		ptNew->tPixelDatas.aucPixelDatas = (unsigned char *)(ptNew + 1);/*加1，首地址加上结构体的大小，指向下一个结构体iVMSize*/

		ptNew->iID = 0;
		ptNew->bDevFrameBuffer = 0;         /*一个普通缓存 */
		...
		/* 放入链表
		*   第一次，g_ptVideoMemHead=NULL   ptNew->ptNext=NULL,   g_ptVideoMemHead   指向ptNew
		*
		*   第二次，新的成员指向链表头指向的成员(也就是第一个成员)，链表头指向新的成员
		* 
		*   所以每次新添加的成员，都在第一个，最早的成员在链表的末尾
		*/
		ptNew->ptNext = g_ptVideoMemHead;
		g_ptVideoMemHead = ptNew;
	}
